...
class map_previewer extends default_previewer
{
	...
	{
		parent::init($id, $page);
		$this->admin_page->module->head_items->add_javascript(JQUERY_URL, true);
		$this->admin_page->module->head_items->add_javascript('//maps.googleapis.com/maps/api/js?sensor=false');
		$this->admin_page->module->head_items->add_javascript('/reason/modules/map/google_maps_V3.js');
	}
	...
	function generate_map_markup()
	{
		...
		if (count($this->mapItems) > 0)
		{
			...
			echo '<p>Add items to this map to see a preview</p>'."\n";
		}
	}
